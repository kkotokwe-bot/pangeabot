#Pol Treestructure- 04/07/2022


setwd("/Users/peggy/Desktop/R scripts for the PANGEA project/Pol Final Treestructure")
...
# read in alignment
alignment_name <- "pol_finalAli_wCGR.fas"
data <- read.dna(alignment_name, format="fasta", as.matrix=FALSE)


# read in tree, add on dates
tr <- read.tree(paste("RAxML_bestTree.ML_BW_", geneRegion, ".nwk", sep=""))
tr2 <- root(tr, outgroup = tr$tip.label[grep("AY371157", tr$tip.label)])
seq <- read.dna(paste(geneRegion, "_finalAli_wCGR.fas", sep=""), format="fasta", as.character = F, as.matrix=F)
...
dt2 <- dater(tr2, sts, s = length(data[[1]]), ###grab the length of the first sequence
  ...
  s = length(tr2$tip.label)

# save the time tree
saveRDS(  dt2, file = paste0("RAxML_bestTree.ML_BW", geneRegion, "_rooted_dated.rds") )

dt2 <- readRDS(paste0("RAxML_bestTree.ML_BW", geneRegion, "_rooted_dated.rds"))
...
write.tree(ts1, paste(geneRegion, "_datedTree.nwk", sep=""))
